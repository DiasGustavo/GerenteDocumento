	...
	public void adicionarListAtos() {
		...
		for (int posi = 0; posi < listaAtosTemp.size(); posi++) {
			...
			for (int pos = 0; pos < listaAtosSelecionados.size(); pos++) {
				...
				if (atosTemp.getId() == ato.getId() && atosTemp.getStatus().equals("Aplicar")) {//Se as idis forem iguais eles serão adicionados como presentes e aplicavéis
					adicionarAtos(ato, "Aplicar");//adiciona os atos como presentes temporariamente
					break;
					//assim que o ato é encontrado é adicionado no checklist
				}
				if (atosTemp.getId() != ato.getId() && atosTemp.getStatus().equals("Aplicar")) {//Se as idis forem diferentes  e o status ativo, os atos não se encontram dentro dos presentes
					diferente = true;														  //nem na situação aplicável 	
				}
				if (pos == listaAtosSelecionados.size()-1) {
					if (diferente == true) {
						listaNSelecionados.add(atosTemp);
						adicionarAtosInativos(atosTemp, "Omitir");//adiciona os atos como ausentes 
					}
				}
			}
